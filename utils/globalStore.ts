...
export const useSwitch: SwitchControls = () => {
  ...
  const setNextSwitchStateIndex = (identifier: string, options: PrevNextSwitchOptions = {}) => {
    const {wrapAround} = {...prevNextSwitchOptions, ...options};
    const current = getSwitchStateIndex(identifier);
    const states = getAllSwitchStates(identifier);

    setSwitchStateIndex(identifier, current + 1 >= states.length ? (wrapAround ? 0 : states.length - 1) : current + 1);
  };

  const setPreviousSwitchStateIndex = (identifier: string, options: PrevNextSwitchOptions = {}) => {
    const {wrapAround} = {...prevNextSwitchOptions, ...options};
    const current = getSwitchStateIndex(identifier);
    const states = getAllSwitchStates(identifier);
  ...
  const getAllSwitchStates = (identifier: string) => {
    return store.__registry && store.__registry[identifier] ? store.__registry[identifier] : [];
  };

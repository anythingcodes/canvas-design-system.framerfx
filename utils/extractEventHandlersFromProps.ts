import {eventTriggerNames, keyEventTriggerNames, automaticEventTriggerNames, isCustomEvent} from './controls';
  ...
  eventTriggerNames
    .reduce((handlers, event) => {
      ...
      const handler = (...args) => {
        ...
        if (isCustomEvent(event) && props[event] && typeof props[event] === 'function') {
          props[event](...args);
        }

        if (action !== 'unset') {
          // check if a trigger condition has been passed in
          if ('shouldTrigger' in props && typeof props.shouldTrigger === 'function' && !props.shouldTrigger(...args)) {
          ...
          handleTrigger(
            ...
            sanitizedIdentifier,
            action,
            action === 'specific-name' ? props[`${event}SpecificName`] : props[`${event}SpecificIndex`],
          );
        }
      };
...
function mergeEvents(map: Map<string, Function[]>, events: {[key: string]: Function}) {
  for (let e in events) {
